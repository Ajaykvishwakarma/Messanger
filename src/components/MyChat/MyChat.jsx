...
export const MyChat = () => {
    ...
    const { notification, unseenmsg } = useSelector((store) => store.notification);
        ...
        <div className="mychat-cont">
            <div>
                <div className="notification">
                    <img src={logo} className="mate_logo"/>
                    <h2>Chats</h2>
            ...
            <div className="recent-chat">
                ...
                <div className="recent-user">
                    {search ? search_result.map((el) => (
                        <SearchUserComp key={el.id} {...el} token={token} recent_chat={recent_chat} setSearch={setSearch} />
                    )) 
                    :
                     !chat_loading && recent_chat.map((el, index) => (
                        <ChatUserComp key={el._id} {...el} index={index} chattingwith={chatting._id} id={user._id} />
                     ))}
                </div>
            </div>
        </div>
...
export const Notificationcomp = () => {
    ...
    const id = open ? "simpale-popover" : undefined;

    return (
        <div>
            <NotificationsIcon aria-aria-describedby={id} onClick={handleClick} />
            <Popover 
                ...
                anchorOrigin={{
                    ...
                    {!unseenmsg.length ? (
                        <Typography sx={{ P: 1, width: 170 }} > No new Messages.</Typography>
...
const ChatUserComp = ({isGroupChat, chatName, users, latestMessage, id, _id, index, chattingwith}) => {
    ...
    return (
        ...
        className={chattingwith == _id ? "user selectUser" : "user"}>
            <div className="history-cont">
                {isGroupChat ?(
                        <div> {<Avatar><GroupIcon /></Avatar>}</div>
                ) : (
                    <div>{<Avatar src={users.find((el) => el._id != id)?.avatar} />} </div>
                )}
                <div>
                {isGroupChat ? (
